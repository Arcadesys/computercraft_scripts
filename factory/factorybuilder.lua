...
-- ------------------------------------------------------------------------
-- CHANGELOG (Refactor):
-- 2025-11-11: Converted to state machine; added DONE state; preserved build logic.
-- ------------------------------------------------------------------------

-- FEATURES (from original retained)
-- - Auto-fuel searching & refuel threshold logic
-- - On-demand material restock attempts
...
local function performInverse(op)
  if op == 'F' then
    -- No reliable turtle.back() digging; use 180 turn + forward + 180 turn
...
states[STATE_RESTOCK] = function(ctx)
  ...
  -- NEW: return to origin to access storage reliably
