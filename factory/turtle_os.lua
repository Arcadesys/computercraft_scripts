...
local function runRefuel()
    ui.clear()
    print("Refueling from inventory...")

    if not turtle or type(turtle.getItemDetail) ~= "function" then
        print("This command must be run on a turtle (turtle API unavailable).")
        return pauseAndReturn("stay")
    end

    local before = nil
    if type(turtle.getFuelLevel) == "function" then
        before = turtle.getFuelLevel()
    end

    local consumed = 0
    -- Iterate slots and attempt to refuel using available items.
    for slot = 1, 16 do
        local detail = turtle.getItemDetail(slot)
        if detail then
            turtle.select(slot)
            -- Keep calling refuel while the slot provides fuel
            while true do
    ...
    -- Restore selected slot to 1
    pcall(turtle.select, 1)

    if before ~= nil and type(turtle.getFuelLevel) == "function" then
        local after = turtle.getFuelLevel()
        print(string.format("Fuel: %s -> %s", tostring(before), tostring(after)))
    end

    print("Consumed items (units): " .. tostring(consumed))
    return pauseAndReturn("stay")
end
...
local function showSystemMenu()
    while true do
        local res = ui.runMenu("System Tools", {
            { text = "Fuel Up", callback = runRefuel },
